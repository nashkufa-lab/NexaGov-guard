...
<style>
    ...
    * {font-family: 'Inter', sans-serif;}
    ...
    .upload-box:hover {border-color: #299D94; background: #F0FDFA;}
    [data-testid="stFileUploader"] {background: white; border-radius: 16px; padding: 8px;}
    .stTabs [data-baseweb="tab-list"] {gap: 8px;}
    .stTabs [data-baseweb="tab"] {background: white; border-radius: 10px; padding: 8px 16px; border: 1px solid #E2E8F0;}
...
with st.sidebar:
    ...
    st.caption("All processing is local. PDFs never leave your session. No data stored.")
    st.markdown("---")
    st.markdown(f"**Live:** nexa-guard.streamlit.app<br/>**Contact:** nash@nexaai.co.zw", unsafe_allow_html=True)

# ===== HEADER =====
if "NexaGov" in brand:
    st.markdown("""
    <div class="brand-header">
        <h1>🏛️ NexaGov Guard</h1>
        <p>Government Procurement Fraud Detection • PRAZ eGP Compliant • Auditor General Benchmarks</p>
    </div>
    """, unsafe_allow_html=True)
else:
    st.markdown("""
    <div class="brand-header" style="background: linear-gradient(135deg, #0F172A 0%, #4338CA 100%);">
        <h1>⚡ NexaAI Audit</h1>
        <p>Private Sector Procurement Intelligence • For CFOs, Internal Audit & Procurement Managers</p>
    </div>
    """, unsafe_allow_html=True)
...
if "NexaGov" in brand:
    ...
    with col_upload:
        ...
        st.caption("Supports: eGP bidding documents, award notices, SBDs")
        if st.button("✨ Load Sample: $658k Fraud Case", use_container_width=True):
    ...
    st.dataframe(df[['tender_ref','item','quantity','price','benchmark','supplier','risk_score','potential_loss']].style.background_gradient(subset=['risk_score'], cmap='Reds'), use_container_width=True, height=300)
    st.caption("🔴 High risk = Overpricing >80% or repeat supplier. For Auditor General report.")
...
# ================= PRIVATE MODE - NEXAAI =================
    ...
    c2.markdown('<div class="metric-card"><div class="label">Invoices Audited Today</div><div class="value">2,847</div><div class="delta">↑ 23% vs yesterday</div></div>', unsafe_allow_html=True)
    c3.markdown('<div class="metric-card"><div class="label">Kickback Risk Detected</div><div class="value">34 cases</div><div class="delta">🔴 Needs action</div></div>', unsafe_allow_html=True)
    ...
    with tab_inv:
        ...
        st.caption("We extract supplier, amount, item and auto-flag overpricing & duplicate suppliers")
        ...
        with col_b:
            ...
            st.caption("Same supplier across categories = possible kickback. Check buyer relationship.")
    
    with tab_quote:
        st.markdown("### 3-Quotation Compliance Check")
        st.caption("Zimbabwe best practice: 3 quotes required. We check if buyer chose cheapest or inflated.")
        ...
        if len(df_q)>=2:
            ...
            if diff > 100:
                st.error(f"🚩 FLAG: Most expensive ({expensive['Supplier']} ${expensive['Amount']:.0f}) vs cheapest ({cheapest['Supplier']} ${cheapest['Amount']:.0f}) = **Overpay ${diff:.0f}**. Ask buyer for written justification - potential kickback.")
                st.markdown(f'<div class="flag-high">Recommendation: Require buyer to justify why not choosing {cheapest["Supplier"]}. Escalate to audit if no valid reason.</div>', unsafe_allow_html=True)
            else:
                st.success("✅ Within tolerance - pricing is competitive")
    
    with tab_csv:
        st.markdown("### Bulk Procurement Audit - CSV")
        st.caption("Upload: supplier, item, amount, buyer, department, date")
...
"""
        ...
        if not csv_file:
            st.info("Demo CSV loaded - upload yours to see buyer risk")
        df_csv['benchmark'] = df_csv['item'].apply(get_benchmark)
        df_csv['overpricing'] = df_csv.apply(lambda r: ((r['amount']/r['benchmark']-1)*100) if r['benchmark'] else 0, axis=1)
        st.dataframe(df_csv, use_container_width=True)
        
        st.markdown("#### 👤 Buyer Risk Heatmap")
        buyer_risk = df_csv.groupby('buyer').agg(total=('amount','sum'), suppliers=('supplier','nunique'), avg_over=('overpricing','mean')).reset_index()
        st.dataframe(buyer_risk, use_container_width=True)
        if 'John Banda' in buyer_risk['buyer'].values:
            st.warning("⚠️ **John Banda**: High spend ($2,080), only 2 suppliers, mostly Best Deal Pvt Ltd - Flag for interview. Possible single-sourcing to favor supplier.")
    
    with tab_report:
        st.markdown("### 📑 CFO / Audit Report - One Click")
        st.caption("Export this for board meeting")
        if st.button("Generate Audit Report", type="primary", use_container_width=True):
            st.success("Report generated!")
            st.markdown("""
            **Executive Summary:**
            - Audited 6 transactions
            - Total spend: $3,140
            - Potential overpayment: $1,240 (39%)
            - 1 buyer flagged (John Banda) - repeat supplier pattern
            - 2 suppliers flagged (Best Deal Pvt Ltd - appears 3x across depts)
            - Recommendation: Rotate buyer John Banda, blacklist Best Deal pending investigation, enforce 3-quote policy
            """)
            st.download_button("📥 Download CFO Report PDF", data="Sample Report", file_name="NexaAI_Audit_Report.pdf")

st.divider()
st.caption(f"{'NexaGov Guard - Government Edition' if 'NexaGov' in brand else 'NexaAI Audit - Private Sector Edition'} • Live: nexa-guard.streamlit.app • Privacy: Local processing only • Harare 🇿🇼")
